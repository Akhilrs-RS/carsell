  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorText || 'Failed to save vehicle details')
      }
              ...
              {!showForm ? (
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-3xl mx-auto shadow-2xl">
                  ...
                  <form onSubmit={handleFormSubmit} className="space-y-6">
                    ...
                    <div>
                      <label className="block text-xs uppercase tracking-wider text-slate-400 font-semibold mb-2">Vehicle Image</label>
                      
                      {carFormData.imageUrl ? (
                        <div className="relative border border-slate-800 rounded-lg p-2 bg-slate-950 flex flex-col items-center">
                          <img 
                            src={carFormData.imageUrl} 
                            alt="Preview" 
                            className="max-h-40 max-w-full object-contain mb-3 rounded"
                          />
                          <button
                            type="button"
                            onClick={() => setCarFormData(prev => ({ ...prev, imageUrl: '' }))}
                            className="px-3 py-1 bg-red-950/40 border border-red-500/30 hover:bg-red-900/50 hover:border-red-500 text-red-400 text-xs font-semibold rounded cursor-pointer transition-colors"
                          >
                            Remove Image
                          </button>
                        </div>
                      ) : (
                        <div className="relative border border-dashed border-slate-800 rounded-lg p-6 bg-slate-950 hover:border-slate-700 transition-colors flex flex-col items-center justify-center cursor-pointer">
                          <input
                            type="file"
                            accept="image/*"
